...
%% 生成质点并判断近场域内的物质点编号
% 左右各加一层虚拟边界，x方向有102个物质点，y方向有50个物质点
nx=L/dx+2;ny=W/dy;
nom=nx*ny;
%第一列存储x坐标，第二列存储y坐标
...
for i=1:ny
    ...
    coordinate_mx((1+nx*(i-1)):(nx*i),2)=ones(nx,1)*dy*i-dy*0.5;    
end
...
for i=1:ny
   force_body(1+102*(i-1),1)=-force_body_rou;
   force_body(102+102*(i-1),1)=force_body_rou;
end
...
for it=1:nt
    ...
    if rem(it,100)==0
        ...
        x=linspace(0.005,0.995,100);ux=1e-3*(x-0.5);
        plot(x,ux,'r--',x,0.5*(displacement(2450:2549,1)+displacement(2552:2651,1)),'g','LineWidth',1.2);
