...
%% 根据实际情况设置边界条件（
[coordinate_x,number_of_point,force_body,number_of_virtual_boundary,node_left,node_right] = determine_boundary(coordinate_x,number_of_point,basic,dimen,dx,dy);
...
%先计算一次原始的形状张量并得到其逆矩阵
[tensor_shape_kb]=calculate_shape_tensor(coordinate_x,list_of_near_point,displacement,number_of_point,dV,delta,dimen,0);
...
for it=1:nt
    %每个时间步先计算变形后的形状张量
    [tensor_shape_ka]=calculate_shape_tensor(coordinate_x,list_of_near_point,displacement,number_of_point,dV,delta,dimen,1);
    %得到变形后的形状张量后计算变形梯度
    ...
    %以应力为准则来破坏键，并计算损伤，参考DOI 10.1007/s10704-016-0126-6
    %[mu,damage]=judge_damage(number_of_point,list_of_near_point,mu,damage,tensor_stress_cauthy,f_critical,dV);
...
%% 导出GIF图
filename = 'D:\code\matlab\bin\math theory\PD\state_base\2d_plane_left&right_nosb\2d_plane.gif';
for idx = 1:imt
