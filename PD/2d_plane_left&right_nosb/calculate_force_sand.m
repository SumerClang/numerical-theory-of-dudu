function [force_fsand] = calculate_force_sand(mu,list_of_near_point,number_of_point,dimen,coordinate_x,displacement,E,dx,dV,delta)
%计算附加力态，此算例中为第一类沙漏力
force_fsand=zeros(number_of_point,dimen);
C1=1e7*E/dx;
%考虑损伤，体力等，计算总的沙漏力
for ii=1:number_of_point
    ...
    for jj=1:28
        ...
        if nn~=0
            ...
            %当键未断开时才进行计算
            if mu(ii,jj)~=0              
                force_sand_it=force_sand_it+omega*C1*yita*dV+omega*C1*yita*dV;                
            end
        end       
    end
